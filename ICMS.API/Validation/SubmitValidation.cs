...
namespace ICMS.API.Validation
{
    public class SubmitValidation
    {
        ...
        {
            return models.All(lo => (stage == ActivityStageCode.PRE && IsValid(lo.LearningObjectiveQuestions)) || (stage == ActivityStageCode.MID && (lo.IsMidActivityOnTarget == true || IsValid(lo.LearningObjectiveStageQuestionAnswers))) || (stage == ActivityStageCode.POST && (lo.IsObjectiveAchieved == true || IsValid(lo.LearningObjectiveStageQuestionAnswers))));
        }
        ...
        private static bool IsValid(LearningObjectiveQuestionAnswerViewModel model)
        {
            ...
            return count >= WordCountValidation.TA_MIN_WC && count <= WordCountValidation.TA_MAX_WC;
        }
        ...
        {
            int count = ICMSMethods.GetWordCount(model.Answer);
            return count >= WordCountValidation.TA_MIN_WC_POSTA && count <= WordCountValidation.TA_MAX_WC_POSTA;
        }
    }
}